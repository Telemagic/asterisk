...
/**
 * return 1 if this is the queue for the agent with longest wait time, 0
 * otherwise (meaning there is another queue that has a waiting call
 * with longer wait time).
 * This will not consider queues with lower priority than this queue.
 */
static int is_longest_wait_time(struct call_queue *rq, struct member *member)
{
	struct call_queue *q;
	struct member *mem;
	int found_older_call = 0;
	struct ao2_iterator queue_iter;

	// The call we are considering - wait time
	time_t rq_start_time = rq->head->start;
	...
	while ((q = ao2_t_iterator_next(&queue_iter, "Iterate through queues"))) {
		...
		if (q->count && q->members) {
			if ((mem = ao2_find(q->members, member, OBJ_POINTER))) {
				...
				if (q->head->start < rq_start_time && q->weight >= rq->weight) {
					ast_debug(1, "Queue '%s' (start %d) has an older entry than candidate '%s' (%d)\n", q->name, q->head->start, rq->name, rq_start_time);
					found_older_call = 1;
				}
				ao2_ref(mem, -1);
			}
...
static int can_ring_entry(struct queue_ent *qe, struct callattempt *call)
{
	...
	if (!is_longest_wait_time(qe->parent, call->member)) {
    	  ast_debug(1, "Other queue has older call %s:%s\n",
    		    qe->parent->name, call->interface);
    	  return 0;
    }
...
static char *__queues_show(struct mansession *s, int fd, int argc, const char * const *argv)
{
	...
	// LEA: Try to skip locking the global queues variable
	//ao2_lock(queues);
